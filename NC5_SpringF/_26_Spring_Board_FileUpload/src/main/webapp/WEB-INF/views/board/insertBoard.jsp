...
<body>
		...
		<form id="insertForm" action="/board/insertBoard.do" method="post"
			...
			<table border="1" style="border-collapse: collapse">
				...
				<tr>
					...
					<td style="text-align: left;"><input type="text"
						name="boardWriter" value="${loginUser.userId }" readonly>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td style="background: skyblue; width: 70px;">파일 첨부</td>
					<td><input type="file" name="uploadFiles" multiple>
						<div id="image_preview">
							<input tpye="file" id="btnAtt" name="uploadFiles"
								multiple="multiple">
							<div id="attZone" data-placeholder="파일을 첨부하려면 파일 선택 버튼을 누르세용"></div>
						</div></td>
				</tr>
				<tr>
	...
	//추가된 파일들을 담아줄 배열. File 객체들이 하나씩 저장.
		...
		$(function({
			...
			$("#btnAtt").on("change", (e)=>{
				...
				//파일 배여랴에 담긴 파일들 하나씩 꺼내서 미리보기 처리
				for(f of fileArr){
					//미리보기 메소드 호출
					imageLoader(f);
				}
	...
	const makeDiv = (img, file) =>{
		...
		let btn = document.cereateElement("input");
		btn.setAttribute("type","button");
		btn.setAttribute("value","x");
		btn.setAttribute("delFile",file.name);
		btn.setAttribute("style","width: 30px; heiht: 30px;" 
		+ " position: absolute; right:0px; botton: 0px;"
		+ " z-index: 999, background-color: rgb(255,255,255,0.1);"
		+ " color: #f00;");
