...
FROM STUDENT ST
         JOIN
     (SELECT MAJOR, SYEAR, MAX(AVR) AS AVR
      FROM STUDENT
      GROUP BY MAJOR, SYEAR
      HAVING MAJOR = '화학'
      ORDER BY MAJOR) MAXAVR
...
SELECT CNO, MAX(RESULT) AS MAXRESULT
FROM C##MG.SCORE
         JOIN (SELECT SNO, SNAME
               FROM STUDENT) ST
              ON ST.SNO = CNO
GROUP BY CNO, SNO

SELECT SC.CNO, C.CNAME, SC.MAXRESULT
FROM COURSE C
         JOIN (SELECT CNO, MAX(RESULT) AS MAXRESULT
               FROM C##MG.SCORE
               GROUP BY CNO, SNO) SC
              ON C.CNO = SC.CNO
...
SELECT SC.SNO, ST.SNAME, MXRS.CNO, C.CNAME, MAXRESULT
FROM SCORE SC
         JOIN(SELECT CNO, MAX(RESULT) AS MAXRESULT
              FROM C##MG.SCORE
              GROUP BY CNO) MXRS
...
-- GROUP BY절에 특정 컬럼을 지정하면 그 컬럼의 값이 같은 데이터끼리
-- 모아서 통계를 내준다.
-- 예로 SCORE 테이블에 CNO을 GROUP BY절에 작성하면
-- CNO이 같은 결국 같은 과목의 데이터만 모아서 통계를 내주는 식으로 진행
...
SELECT MS.DNO, DNAME, MS.MINSAL
FROM EMP
         JOIN (SELECT DNO, MIN(SAL) AS MINSAL
               FROM EMP
               GROUP BY DNO) MS
              ON MS.DNO = EMP.DNO
...
SELECT MS.DNO, D.DNAME, MS.MINSAL
FROM EMP E
         JOIN(SELECT DNO, MIN(SAL) AS MINSAL
              FROM EMP
              GROUP BY DNO) MS
             ON E.DNO = MS.DNO AND E.SAL = MS.MINSAL
         JOIN DEPT D on E.DNO = D.DNO

-- 부서별 연봉 최저값 부서명, 사원이름 포함(잘못된 GROUP BY)
-- 공통된 값이 없는 칼럼을 GROUP BY하며 그룹화의 의미가 없어진다
SELECT MNSL.DNO, D.DNAME, E.ENO, E.ENAME, MNSL.MINSAL
FROM (SELECT DNO, MIN(SAL) AS MINSAL
      FROM EMP
      GROUP BY DNO) MNSL
         JOIN EMP E
              ON E.DNO = MNSL.DNO AND E.SAL = MNSL.MINSAL
         JOIN DEPT D on E.DNO = D.DNO

-- 잘못된 GROUP BY
-- SELECT절에 컬럼이 하나 추가되는 순간 GROUP BY에도 추가해줘야하기 때문에
...
-- 학생별 최대 점수
SELECT SNO, MAX(RESULT) AS MAXRES
FROM C##MG.SCORE
GROUP BY SNO

-- 학생별 최대 점수의 과목번호
SELECT MSRS.SNO, SC.CNO, MSRS.MAXRES
FROM (SELECT SNO, MAX(RESULT) AS MAXRES
      FROM C##MG.SCORE
      GROUP BY SNO) MSRS
         JOIN C##MG.SCORE SC
              ON MSRS.SNO = SC.SNO
                  AND MSRS.MAXRES = SC.RESULT
ORDER BY SNO, CNO
...
-- 1-4 COUNT
-- 각 학과별 학년별 학생수
SELECT MAJOR, SYEAR, COUNT(SYEAR)
FROM STUDENT
GROUP BY MAJOR, SYEAR;
-- 각 학과별 교수의 수
