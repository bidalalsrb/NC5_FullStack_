...
-- STUDY계정에 DBA권한이 있어서 SYS가 소유하고 잇는 테이블도 사용할 수 있다.
...
-- ALTER SYSTEM KILL SESSION 'SID NO,SERIAL NO' IMMEDIATE'(즉시실행)'
...
-- 수식으로 만든 인덱스는 컬럼명이 표시되지 않고 시스템에서
-- 자동으로 생성한 수식의 이름을 사용한다.
...
CREATE OR REPLACE VIEW V_AVG_SCORE
            (
             CNO, CNAME, MAJOR, AVGRESULT --컬럼명을 커스터마이징해서 사용할 수 있기 때문에
            -- 원천 테이블의 컬럼명을 노출하지 않을 수 있음.
                )
AS
(
-- 위에서 지정한 컬럼의 순서와 개수가 일치해야 한다.
SELECT SC.CNO, C.CNAME, ST.MAJOR, ROUND(AVG(SC.RESULT), 2)
FROM COURSE C
...
-- 조회쿼리가 1학년만 가져오는 쿼리라서 원천 테이블 추가된 2,3,4학년의
-- 데이터는 표출되지 않는다.
CREATE OR REPLACE VIEW ST_CH(SNO, SNAME, SYEAR, AVR)
AS
(
...
SELECT ROWNUM, A.MAJOR, A.RESULT
FROM (SELECT ST.MAJOR, ROUND(AVG(RESULT), 2) AS RESULT
      FROM STUDENT ST
               JOIN SCORE S2 on ST.SNO = S2.SNO
      GROUP BY ST.MAJOR
      ORDER BY RESULT DESC) A
WHERE ROWNUM <= 3;
...
SELECT B.*
FROM (SELECT ROWNUM AS RO, A.ENO, A.ENAME, A.SAL
      FROM (SELECT ENO, ENAME, SAL
            FROM EMP
...
-- 옵션있는 시퀀스의 사용
-- 시퀀스가 가르키는 값을 다음 값으로 옮긴다
INSERT INTO EMP_COPY1
VALUES (EMP_CO_ENO_SEQ.NEXTVAL, 'G', '개발', 0, SYSDATE, 3000, 100, 0);
...
-- 시퀀스의 다음 값을 조회
-- NEXTVAL 호출 시 무조건 시퀀스가 다음값으로 세팅됨
