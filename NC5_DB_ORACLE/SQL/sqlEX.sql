...
select P.*
from PROFESSOR P
         join COURSE C2 on P.PNO = C2.PNO and SECTION = '화학';

--5) 화학과 1학년 학생의 기말고사 성적을 검색한다
select p.SECTION, s.SNAME, s.AVR
from PROFESSOR P
         join STUDENT s on p.SECTION = s.MAJOR and s.SYEAR = '1' and p.SECTION = '화학';

--6) 일반화학 과목의 기말고사 점수를 검색한다
select sc.CNO, c.CNAME, sc.RESULT
from C##MG.SCORE sc
         join COURSE c on c.CNO = sc.CNO and c.CNAME = '일반화학';
...
select s.*
from STUDENT S
         join STUDENT s2 on s.SNAME = s2.SNAME and s.SNO != s2.SNO
order by s.SNAME;
...
select C.CNAME, p.PNAME, C.ST_NUM
from PROFESSOR P
...
select c2.CNO, p.PNAME, c2.CNAME
from PROFESSOR p
         left join COURSE C2 on p.PNO = C2.PNO;

--2) 화학과 학생의 기말고사 성적을 모두 검색하라
select S.*, s2.RESULT
from STUDENT S
         left join SCORE S2 on S.SNO = S2.SNO and s.MAJOR = '화학';
...
select distinct s3.MAJOR, p.PNAME
from PROFESSOR P
         join COURSE C2 on P.PNO = C2.PNO
