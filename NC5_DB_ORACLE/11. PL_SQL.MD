...
2. 데이터 타입은 VARCHAR2, NUMBER, DATE ... 기본 데이터 타입과 테이블명.컬럼명%TYPE, 테이블명, ROM%TYPE.. 등 선언가능.
3. 레코드의 선언 방법
    - TYPE 레코드명 IS RECORD(
...
2. 연관배열 선언 방법
    - TYPE 배열명 IS TABLE OF 데이터타입[NOT NULL, DEFAULT]
      INDEX BY 인덱스의 형태(BUNARY_INTEGER, PLS_INTEGER, VARCHAR2...)
3. 연관배열 사용 방법
    - 배열변수이름 배열명;
    - 배열변수이름(INDEX)'
4. 연관배열의 메소드
    - EXISTS(INDEX) : 헤당 INDEX에 데이터가 존재하는 지 여부를 판단. TRUE/FALSE 리턴.
    - COUNT : 배열에 저장된 데이터의 개수를 반환해주는 메소드.
    - LIMIT : 배열의 최대 크기 반환
    - FIRST : 배열의 첫번째 인덱스 번호 반환.
    - LAST : 배열의 마지막 인덱스 번호 반환.
    - PRIOR(INDEX) : 해당 인덱스 바로 이전 INDEX  반환. INDEX가 없으면 NULL을 반환
    - NEXT(INDEX) : 해당 INDEX 바로 뒤 INDEX를 반환. 바로 뒤 INDEX가  없으면 NULL을 반환.
...
6. 커서 파라미터 
    ...
    - 사용 : OPEN 커서명(데이터타입 값) 
7. 묵시적 커서
    - 커서를 선언하지 않고 쿼리문을 실행했을 때 오라클 내부에서 결과를 저장하게 되는데 오라클 내부에 저장된 결과를 사용할 수 있는 커서
    - DML(INSERT, DELETE, UPDATE) 같은 경우도 영향을 받은 행의 개수를 저장하고 있는데 묵시적 커서를 사용하면 영ㄹ향받은 행긔 개수를 가져올 수도 있다.
    - SQL%NOTFOUND, SQL%FOUND, SQL%ROWCOUNT, SQL%ISOPEN 속성들을 사용할 수 있다.

## 7. 예외처리
1. PL/SQL에서 정상적으로 동작이 수행되지 않을 때 오류가 발생했을 때 EXCEPTION으로 예외처리부를 작성하여 오류를 처리해야 한다.
2. 예외처리부에는 대부분 ROLLBACK; 처리해서 위에서 실행된 쿼리문이 적용되지 않도록 트랜잭션을 취소한다.
3. 예외 종류
    - 내부예외
        - 사전 정의된 예외 : 내부 예외중에 예외번호를 지정한 이름이 존재하는 예외.
        - 이름이 없는 예외 : 내부 예외중에 이름이 없는 예외 (사용자가 경우에 따라서 이름을 지정해 줄 수 있다.)
...
4. 자주 사용되는 사전 정의 예외
    ...
    - COLLECTION_IS_NULL : ORA-06531. 초기화되지 않은 배열(값이 할당되지 않은 배열) 사용 했을 경우
    ...
    - INVALID_NUMBER : 숫자타입에 숫자타입이 아닌 값을 저장할 경우
    ...
    - TIMEOUT_ON_RESOURCE : ORA-00051, 지원 대기시간 초과했을 경우
